...
export const planSpec: {
    ...
    Trial: {
        ...
        description_warning: ["All user data is deleted every 14 days"],
    },
    Standard: {
        // price: {
        // 	monthly: 19,
...
export class ServiceSpec {
    ...
    constructor(service: Service) {
        ...
        this.servicePlan.product_price =
            planSpec[this.plan]?.product_price || {};
        this.servicePlan.users =
            this.plan === "Unlimited"
                ? "Unlimited"
                : this.getUserSize(planSpec[this.plan].users as number);
        this.servicePlan.storage.database =
            this.plan === "Unlimited"
                ? "Unlimited"
                : this.getDataSize(
                      planSpec[this.plan].storage.database as number
                  );
        this.servicePlan.storage.cloud =
            this.plan === "Unlimited"
                ? "Unlimited"
                : this.getDataSize(planSpec[this.plan].storage.cloud as number);
        this.servicePlan.storage.host =
            this.plan === "Unlimited"
                ? "Unlimited"
                : this.getDataSize(planSpec[this.plan].storage.host as number);
        this.servicePlan.storage.email =
            this.plan === "Unlimited"
                ? "Unlimited"
                : this.getDataSize(planSpec[this.plan].storage.email as number);
    ...
    async getStorage(): Promise<{
        ...
        if (this.service.service.subdomain) {
            ...
            wait.push(
                skapi.util
                    .request(
                        this.service.admin_private_endpoint + "host-directory",
                        {
        ...
        wait.push(
            skapi.util
                .request(
                    this.service.record_private_endpoint + "storage-info",
                    { service: this.service.id, owner: this.service.owner },
                    { auth: true }
                )
                .then((r) => {
